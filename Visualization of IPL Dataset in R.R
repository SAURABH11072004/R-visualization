...
#IPL_RAW is a raw dataset it has to be cleaned.
IPL_RAW<- read_csv("C:/Users/chand/Downloads/IPL_RAW.csv")
View(IPL_RAW)

#IPL is a clean dataset after performing following operation.
IPL<- read_csv("C:/Users/chand/Downloads/IPL_RAW.csv")
View(IPL)
...
ggplot(IPL, aes(x = season, fill = winner)) +
...
ggplot(IPL, aes(x = toss_winner == winner)) +
...
ggplot(IPL, aes(x = target_runs, y = result_margin)) +
...
seasonal_runs <- IPL %>% group_by(season) %>% summarize(total_runs = sum(target_runs, na.rm = TRUE))
...
ggplot(IPL, aes(x = winner, y = result_margin)) +
...
ggplot(IPL, aes(x = season, fill = toss_winner == winner)) +
  geom_bar(position = "dodge") +
...
avg_win_margin <- IPL %>% group_by(winner) %>% summarize(avg_margin = mean(result_margin, na.rm = TRUE))
...
ggplot(IPL, aes(x = match_type, y = result_margin)) +
...
top_venues <- as.data.frame(table(IPL$venue))
...
ggplot(IPL, aes(x = season, y = target_overs, fill = season)) +
